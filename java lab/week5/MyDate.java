...
public class MyDate {
	private int day;
	private int month; // should be incremented by one before printing
	private int year;

	static int[] maxDays = {31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};


	private static int getMaxDayOfAMonth(int month, int year){
		int day = maxDays[month];
		if(month ==1 && day ==29 && !inLeapYear(year)){
			day =28;
		}
		return day;
	}
	...
	public void incrementMonth(int i) {
		...
		month = newMonth;
		year += yearChange;
		if (day > getMaxDayOfAMonth(month, year)){
			day = getMaxDayOfAMonth(month, year);
		}
	}
	...
	public static boolean inLeapYear(int year ){
		return year % 4 == 0 ? true : false;
	}
